...
if (/^usosweb\.[^.]+\.edu\.pl$/.test(location.hostname)) {
  ...
  function addGradeEntry(type: "defeated" | "fail", entry: GradeEntry): void {
    const stats = loadStatistics();
    const list =
      type === "defeated" ? stats.defeatedEntries : stats.failEntries;
    list.push(entry);
    saveStatistics(stats);
  }
  ...
  function getFrameId(frame: Element): string {
    ...
    const titleLink = frame.querySelector(
      '[slot="title"] a, h2[slot="title"] a',
    );
  ...
  `,
      ...
      if (
        ...
        chrome.storage.sync.get("betterUsosSettings", (res) => {
          const settings = (res?.betterUsosSettings ??
            {}) as BetterUsosSettings;
  ...
  async function setupHiddenGrades(): Promise<void> {
    try {
      ...
      const getSubjectName = (): string => {
        ...
        if (link)
          return (link.textContent ?? "").trim() || "Nieznany przedmiot";
      ...
      gradeCells.forEach((cell, index) => {
        ...
        if (!isPerfect && Math.abs(gradeValue) < 0.005)
